...
## 22. AJD op een bestaand pand — de openstaande vraag uit §21, beslist

**De vraag.** §21 signaleerde dat dit model op elke aankoop zowel ITP (10%) als AJD (1,5%) rekende, terwijl die twee in Spanje elkaars alternatief zijn: ITP op de overdracht van een bestaand pand, AJD (naast btw) op een nieuwbouwakte. De module heet zelf "Spain, existing build" — wat ITP impliceert, geen AJD op de koopakte. Onafhankelijke verificatie over twaalf actuele bronnen (2025-2026) bevestigt dat: AJD wordt alleen geheven bij nieuwbouw, of bij de hypotheekakte — en die laatste is sinds de hervorming van 2018 voor rekening van de bank, niet van de koper. Op de koopakte van een bestaand pand betaalt de koper geen AJD.

**De correctie.** `chargedRates()` in `acquisition.ts` is de ene plek waar het model vastlegt wat een bestaande-bouw-aankoop werkelijk kost; `stampDutyAJD` staat daar nu op nul, terwijl `ACQUISITION_RATES.value.stampDutyAJD` zelf blijft staan — het tarief is correct en gesourced, alleen niet verschuldigd op déze transactie. Zowel `acquisitionCosts()` (die een concrete deal prijst) als `acquisitionCostRates()` (die de kopregel op de gratis indicatie voedt) lezen `chargedRates()`, dus konden en kunnen de twee getallen niet meer uit elkaar lopen — dezelfde discipline als bij de aankoopkosten-kopregel zelf.

Er bestaat in de code geen onderscheid tussen nieuwbouw en bestaande bouw — "existing build" is de enige weg door de engine, bevestigd met een controle over de hele codebase. `stampDutyAJD` is daarom niet verwijderd maar op nul zichtbaar gehouden, met een verwijzing naar deze paragraaf: de dag dat een nieuwbouw-pad wordt toegevoegd, staat het tarief er correct gesourced klaar.

**Bewuste divergentie van het bronmodel.** Costs & Income!B125:H153 rekent AJD wél mee op de volle koopsom, naast ITP, voor elke aankoop. Dat is dezelfde soort fout als bij de fiscale-woonplaats-correctie: het werkblad bevat de dubbeltelling, en hem overnemen zou betekenen dat een getal wordt gepubliceerd waarvan al bekend is dat het onjuist is. Deze correctie wijkt daarom bewust af.

**Het nieuwe percentage.** Verplicht (ITP 10% + notaris 0,5% + registratie 0,3% + juridisch 1,0%): **11,8%**, niet 13,3%. Met bemiddeling: **16,8%**, niet 18,3%. Handmatig na te rekenen: € 250.000 koopsom → € 29.500 verplichte kosten, niet € 33.250.

**Twee effecten, niet één.** De correctie is geen zuivere besparing. AJD maakte ook deel uit van de aftrekbare verkrijgingswaarde in `exit.ts` (`acquisitionValueForCapitalGainsTax`); een lagere aankoopkost betekent dus een hogere belastbare meerwaarde bij verkoop. Beide effecten zijn nu gemodelleerd: minder eigen vermogen nodig bij aankoop, meer vermogenswinstbelasting bij verkoop. Het nettoresultaat op de referentiecasus is positief — de besparing op de instapkosten weegt zwaarder dan de extra belasting jaren later, verdisconteerd.

**Gevolg voor de referentiecasus** (Avenida Primado Reig 19, basisscenario): benodigd eigen vermogen van € 197.990 naar € 193.040, totale aankoopkosten van € 445.490 naar € 440.540. TSG-score van 3,6 naar 3,7, percentiel van 68 naar 70. De referentieverdeling uit SCORE_SPEC.md §5 is opnieuw gegenereerd, zoals de "Verversing"-regel daar voorschrijft; de bandbreedte van de verdeling zelf (minimum 0,3, mediaan 2,6, maximum 6,8) is ongewijzigd — de correctie verschuift individuele casussen binnen die band, niet de band zelf.
