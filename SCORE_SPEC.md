...
> **Randgeval bij de 4,0-drempel.** De omzetting gradeert de afgeronde score (§1: één decimaal), want de tabel hierboven is over één-decimaalwaarden geschreven. Gevolg: "Gemiddeld" houdt ook enkele euro's onder break-even stand. Op het §2.1-segment van −€ 250 (2,0) naar € 0 (4,0) rondt een ruwe score pas bij −€ 6,25 naar 3,9 af; dáár begint Laag feitelijk. Het alternatief — graderen op de ruwe interpolatie — legt de grens exact op € 0, maar wijkt dan af van de één-decimaalbanden in deze tabel én van de afgeronde dimensiescore die het betaalde rapport voor hetzelfde pand publiceert. **Dit gold zolang de cashflow-indicatie vóór financiering was berekend — zie de voetnoot bij §8.4 voor wat fase A stap 3 en 4 daaraan veranderden.**
...
> **Voetnoot (fase A stap 3/4):** de cashflow-indicatie hierboven was vóór financiering; sinds fase A stap 3 rekent ze na financiering (vaste aanname: FINANCING_STRATEGIES.medium, 70% LTV/20 jaar, plus NON_RESIDENT_INTEREST_SPREAD, 3,85% effectief). Voor deze referentiecasus verandert dat het middelpunt van € 590,24 naar € −875,12/maand — de curve klemt daar vast op 0,0, en het label wordt **Laag**, niet Hoog. Dat bleek geen incident: op een representatieve steekproef van het gratis pad se eigen invoerbereik (13 wijken × prijzen € 80.000–€ 600.000 in stappen van € 20.000 × oppervlaktes 30–200 m² in stappen van 10 m², 6.318 casussen) klemde 62,4% net zo vast, wat de oude gedeelde drempel (medium 4,0) bijna alles "Laag" liet noemen. Fase A stap 4 heeft daarom `FREE_TIER_INDICATIVE_LABEL_THRESHOLDS` gesplitst in een cashflow-paar (medium 1,0, high 7,0) en een datazekerheid-paar (ongewijzigd 4,0/7,0) — zie §8.5 en de parameter se eigen `reasoning`-veld in parameters.ts voor de volledige herleiding. Op dezelfde steekproef geeft dat 67% Laag / 24% Gemiddeld / 9% Hoog. Deze referentiecasus zelf blijft Laag: € −875,12/maand zit ver onder zelfs de verlaagde grens, en dat is de juiste uitkomst, niet een teken dat de herijking mislukte.
...
De vier drempelwaarden uit §8.2 staan als één ESTIMATE-parameter in parameters.ts (`FREE_TIER_INDICATIVE_LABEL_THRESHOLDS`), niet inline in de rekenlaag — CLAUDE.md §6 laat geen hardgecodeerde getallen toe, ook niet als ze verder nergens vandaan komen. Ze zijn een productdefinitie (waar TSG de grens tussen de labels legt), wat precies is waar ESTIMATE voor bedoeld is. Sinds fase A stap 4 is de waarde genest in twee onafhankelijk gekalibreerde paren, `{ cashflow: {medium, high}, dataConfidence: {medium, high} }` — cashflow op 1,0/7,0, datazekerheid ongewijzigd op 4,0/7,0 — zodat het herijken van het ene paar het andere niet ongewild meebeweegt (`toIndicativeLabel()` accepteert het te gebruiken paar nu als eigen argument in plaats van de parameter zelf te lezen). Zie de parameter se eigen `reasoning`-veld voor de volledige herleiding, inclusief de steekproef die de nieuwe cashflow-waarden onderbouwt.
